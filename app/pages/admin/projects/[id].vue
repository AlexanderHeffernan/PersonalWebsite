...
const { data: project, error: fetchError, refresh } = await useFetch<Project>(`/api/admin/projects/${projectId}`)
...
const handleSave = async () => {
  ...
  try {
    ...
    error.value = err.data?.message || 'Failed to save project'
  } finally {
    saving.value = false
  }
}
...
const deleteImage = async (imageId: number) => {
  ...
  try {
    ...
    error.value = err.data?.message || 'Failed to delete image'
  }
}
